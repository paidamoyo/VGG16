...
class ConvVae:
    ...
    def _create_loss_optimizer(self, epsilon=1e-8):
        ...
        recon = 125*int(np.power(10, 7))*self.flags['image_dim']*self.flags['image_dim'] * tf.reduce_sum(tf.squared_difference(self.x, self.x_recon))
        #else:
        # recon = (tf.reduce_sum(-self.x * tf.log(self.x_recon + epsilon) - (1.0 - self.x) * tf.log(1.0 - self.x_recon + epsilon)))
        vae = float(np.power(10, -7)) *tf.reduce_sum(0.5 * (tf.square(self.mean) + tf.square(self.stddev) - 2.0 * tf.log(self.stddev + epsilon) - 1.0))
