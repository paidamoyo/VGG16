...
class ConvVae:
    ...
    def _decoder_BREAST(self, z):
        ...
        decoder.deconv2d(5, 1, activation_fn=tf.nn.tanh, s_value=0.2)
        return decoder.get_output(), mean, stddev
    ...
    def _create_loss_optimizer(self, epsilon=1e-8):
        ...
        recon = 125*1000000000 * self.flags['image_dim']*self.flags['image_dim'] * tf.reduce_sum(tf.squared_difference(self.x, self.x_recon))
        #else:
        # recon = (tf.reduce_sum(-self.x * tf.log(self.x_recon + epsilon) - (1.0 - self.x) * tf.log(1.0 - self.x_recon + epsilon)))
        vae = 0.0000000001 * tf.reduce_sum(0.5 * (tf.square(self.mean) + tf.square(self.stddev) - 2.0 * tf.log(self.stddev + epsilon) - 1.0))
