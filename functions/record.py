...
def setup_metrics(flags):
    print_log('Date: ' + str(datetime.datetime.now()).split('.')[0])
    datasets = 'Datasets: '
    ...
    for l in range(len(flags['lr_iters'])):
        print_log('EPOCH %d' % l)
        print_log('Learning Rate: ' + str(flags['lr_iters'][l][0]))
        print_log('Iterations: ' + str(flags['lr_iters'][l][1]))
